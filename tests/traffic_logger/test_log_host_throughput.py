...
class TrafficLoggerHostThroughputViewTest(TestCase):
    def setUp(self):
        # =============================
    ...
    def test_non_post(self):
        response = self.c.get('/traffic-logger/host/')
        self.assertEqual(response.status_code, HTTPStatus.METHOD_NOT_ALLOWED)
        self.assertEqual(response['Allow'], 'POST')

    def test_bad_content_type(self):
        response = self.c.post('/traffic-logger/host/', data={'a':"fishsticks"})
        self.assertEqual(response.status_code, HTTPStatus.BAD_REQUEST)

    def test_bad_cbor(self):
        data = b'i-am-garbage-hear-me-r0ar'
        response = self.c.post('/traffic-logger/host/',
                               data=data,
